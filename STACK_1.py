...
class MyApp(object):
    def __init__(self, parent):
        ...
        close_btn = tk.Button(
            self.frame, text="Close Application", command=self.closeApplication
        )
        close_btn.pack()
        ...
        tk.Button(
            self.root, text="Confirm Planets", command=self.confirmSelected
        ).place(relx=0.205, rely=0.62)

        Executefile_btn = tk.Button(
            self.root, text="Run rando file", command=self.runfile
        )
